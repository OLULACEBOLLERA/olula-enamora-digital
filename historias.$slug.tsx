      ...
      <div className="mt-10 gap-10 lg:grid lg:grid-cols-[320px_1fr] lg:items-start">
        <figure className="mx-auto max-w-sm lg:sticky lg:top-24 lg:mx-0 lg:max-w-none">
          <img
            src={h.imagen}
            alt={h.imagenAlt}
            width={h.imagenAncho ?? 202}
            height={h.imagenAlto ?? 405}
        ...
        <div className="mt-10 lg:mt-0">
          ...
          {h.huecosFoto && h.huecosFoto.length > 0 && (
            <div className="mb-8 grid gap-4 sm:grid-cols-2">
              {h.huecosFoto.map((hf) => (
                <div
                  key={hf.titulo}
                  className="flex aspect-[4/3] flex-col items-center justify-center rounded-2xl border-2 border-dashed border-muted-foreground/40 bg-secondary/40 p-4 text-center"
                >
                  <p className="text-xs font-bold uppercase tracking-widest text-turquesa-dark">
                    📷 {hf.titulo}
                  </p>
                  <p className="mt-2 text-xs text-muted-foreground">{hf.descripcion}</p>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="tarjeta mt-6 p-6 sm:p-7">
            <h2 className="titular text-xl">{h.tituloFuentes ?? "Fuentes originales"}</h2>
